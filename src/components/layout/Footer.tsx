...
const Footer = () => {
    ...
    return (
        <footer className="footer">
                ...
                <nav className="footer-nav">
                    <div className="footer-nav-track">
                        ...
                        {navItems.map((item, index) => (
                            <Link key={`set1-${index}`} href={item.href}>{item.name}</Link>
                        ))}
                        {/* Duplicate set for seamless loop */}
                        {navItems.map((item, index) => (
                            <Link key={`set2-${index}`} href={item.href}>{item.name}</Link>
                        ))}
                    </div>
                </nav>
            </div>
            ...
            <a href="/legal" className="footer-legal">THE LEGAL STUFF</a>
        </footer>
    );
};
